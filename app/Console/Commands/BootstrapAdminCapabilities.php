...
class BootstrapAdminCapabilities extends Command
{
    ...
    public function handle(BootstrapSystemCapabilities $action): int
    {
        ...
        try {
            $beforeGrantsCount = \App\Models\UserSystemCapabilityGrant::where('user_id', $user->id)
                ->whereIn('capability', BootstrapSystemCapabilities::PACKAGE)
                ->where('is_active', true)->count();
            
            $action->execute($user);
            
            $afterGrantsCount = \App\Models\UserSystemCapabilityGrant::where('user_id', $user->id)
                ->whereIn('capability', BootstrapSystemCapabilities::PACKAGE)
                ->where('is_active', true)->count();
