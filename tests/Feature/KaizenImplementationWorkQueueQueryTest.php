...
class KaizenImplementationWorkQueueQueryTest extends TestCase
{
    ...
    public function test_overdue_calculation_dynamic_based_on_target_date_and_timezone()
    {
        ...
        $this->assertTrue($keyed[$yesterday->id]->is_overdue, 'Yesterday should be overdue');
        $this->assertFalse($keyed[$today->id]->is_overdue, 'Today is not overdue');
        $this->assertFalse($keyed[$tomorrow->id]->is_overdue, 'Tomorrow is not overdue');
        $this->assertFalse($keyed[$noDate->id]->is_overdue, 'No date is not overdue');
    }
