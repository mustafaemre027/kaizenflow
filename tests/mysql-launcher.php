...
if (!isset($env["DB_USERNAME"])) {
    ...
    foreach ($envLines as $line) {
        if (strpos($line, "DB_USERNAME=") === 0) $env["DB_USERNAME"] = trim(explode("=", $line, 2)[1]);
        if (strpos($line, "DB_PASSWORD=") === 0) $env["DB_PASSWORD"] = trim(explode("=", $line, 2)[1]);
        if (strpos($line, "DB_HOST=") === 0) $env["DB_HOST"] = trim(explode("=", $line, 2)[1]);
        if (strpos($line, "DB_PORT=") === 0) $env["DB_PORT"] = trim(explode("=", $line, 2)[1]);
    }
}
